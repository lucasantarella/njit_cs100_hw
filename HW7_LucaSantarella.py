...
def litCricFriend(wordList, text):
    """ returns frequency of words in wordsList contained in text
    ...
    words_sanitized = []
    for word in words:
        words_sanitized.append(word.strip(string.punctuation))
    ...
    word_count = 0
    for wordq in wordList:
        word_count += words_sanitized.count(wordq.lower())
    ...
    return word_count / len(words_sanitized)
...
# each function call, identifying what it is.theBells For example, it might say

# >>> bellsAAnFrequency 0.07265587064676617.
print('Frequency of \'a\' & \'an\' in `theBells`: ' + str(litCricFriend(['a', 'an'], theBells)))
print('Frequency of \'the\' in `theBells`: ' + str(litCricFriend(['the'], theBells)))
print('Frequency of \'a\' & \'an\' in `cantoXII`: ' + str(litCricFriend(['a', 'an'], cantoXII)))
print('Frequency of \'the\' in `cantoXII`: ' + str(litCricFriend(['the'], cantoXII)))
...
The results show that Poe uses `a` & `an` 3.3x more frequently than Neruda does.
The results also show that Poe uses `the` 2.6x more frequently than Neruda does
'''
